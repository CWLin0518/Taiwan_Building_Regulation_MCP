    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      try {
        if (request.params.name === 'search_building_code') {
          ...
          const update = await ensureDatabaseCurrent();
          if (update.updated && update.data) {
            this.lawData = update.data;
          }

          if (!this.lawData) {
            try {
              const articles = await readDomainDatabase();
              this.lawData = {
                lawName: [...new Set(articles.map((article) => article.lawName))].join('、'),
                lastUpdated: articles[0]?.retrievedAt ?? '',
                articles,
              };
            } catch {
              this.lawData = await fetchLawData(true);
              await writeKnowledgeDatabase(this.lawData);
            }
          }
          ...
          const update = await ensureDatabaseCurrent(true);
          this.lawData = update.data ?? this.lawData ?? await fetchLawData();
          return {
            content: [{ type: 'text', text: update.updated
              ? `偵測到法規變更，已更新 ${this.lawData.articles.length} 條並重建專案 domain/skill 資料庫。`
              : `已完成版本檢查；官方條文無變更，目前共 ${this.lawData.articles.length} 條。` }],
          };
        } else {
